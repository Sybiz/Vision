Public Class CustomFieldAddorEdit
  ...
  Public Sub Invoke(ByVal e As Sybiz.Vision.Platform.Admin.Breakpoints.BreakpointCustomRibbonButtonClickEventArgs) 'Do not remove - SYBIZ
	...
	If e.Key = "TransactionCustomFieldExample" Then
		Try
			      
			'This example looks at the purchase header custom field list; looking at admin.ExtendedPropertyTemplate will let you see what options you hav
			'The same principles apply when updating line based custom fields such as Purchase Line
			Dim templateId As Integer = BreakpointHelpers.ExecuteScalarCommand(CommandType.Text, "SELECT ExtendedPropertyTemplateId FROM admin.ExtendedPropertyTemplateExpanded WHERE ObjectType = 'Purchase Header'", 60,Nothing)
			Dim purchaseTransactionsCustomFieldList As Sybiz.Vision.Platform.Admin.ExtendedPropertyGroupTemplate = Sybiz.Vision.Platform.Admin.ExtendedPropertyGroupTemplate.GetObject(templateId)
			
			'Variable could be passed in here to look for a specific custom field, or some modification to the query to check for it being a certain type, or similar.
			'We are searching for any value; if nothing returned, it is assumed that the custom field does not exist.
			Dim customFieldId As Integer? = BreakpointHelpers.ExecuteScalarCommand(CommandType.Text,"SELECT ExtendedPropertyTemplateItemId FROM admin.ExtendedPropertyTemplateItem WHERE Name = 'BreakpointCustomField'",60,Nothing)
			
			'Below code is an example only to demonstrate to create a basic custom field against a transaction header/line							        
			If customFieldId.HasValue = False Then
				Dim newCustomField As Sybiz.Vision.Platform.Admin.ExtendedPropertyTemplateGroupItem = Sybiz.Vision.Platform.Admin.ExtendedPropertyTemplateGroupItem.NewObject(purchaseTransactionsCustomFieldList.Id, purchaseTransactionsCustomFieldList.Tabs)
				Dim transactionType As Sybiz.Vision.Platform.Admin.ExtendedPropertyGroupTransactionType
				purchaseTransactionsCustomFieldList.TemplateGroupItems.Add(newCustomField)
				...
				'Need to mark specific transaction types as inactive if not used on all transaction types, defaults to all transactions
				'This is instead of marking the custom field as active, to make it inactive all transaction types must be marked as false
				For Each transactionType In newCustomField.TransactionTypes
					If transactionType.Text <> "PurchaseOrder" Then
			...
			'Specifically need to save the "header", not the child object.
			purchaseTransactionsCustomFieldList = purchaseTransactionsCustomFieldList.Save()
		Catch ex As Exception
			BreakpointHelpers.ShowErrorMessage(e.Form, "DANGER WILL ROBINSON!", ex.Message)
		End Try
	End If
